% !TeX root = main.tex
% HWI bachelor document template
% author: Prof. Dr.-Ing. Volker Skwarek
% contact: volker.skwarek@haw-hamburg.de
\newcommand{\templateversion}{HWI-Template v5 11.10.2021}
%---------------------------------------

%---------------------------------------
% in order to make this document more readable
% all document setting which better shall NOT
% be changed are moved into the preamble file
%---------------------------------------
% it may happen for compatibility reasons of 
% individually added packagesm that this 
% preamble needs to be changed nevertheless
%---------------------------------------
% but this is, what LaTeX is and it is for...
%---------------------------------------

%---------------------------------------
% variables for document personalisation
%---------------------------------------
\newcommand{\thesistype}{Masterarbeit}
% Bachelorarbeit
% Projektarbeit
% Masterarbeit
% Seminararbeit
% Versuchsbericht
\newcommand{\thesistitle}{Optimization of on-demand line-based bus services}
\newcommand{\student}{Alexander Klaus}
\newcommand{\studentnumber}{7169020}
\newcommand{\firstsupervisor}{Prof. Dr. Knut Haase}
\newcommand{\secondsupervisor}{Prof. Dr. Malte Fliedner}
\newcommand{\thesisdate} {27. August 2025}
%---------------------------------------

\input{_basic_settings_dont_touch/preamble}

%-------------------------------------
\addbibresource{Meine_Bibliothek.bib} %Bibliographiedateien laden

%-------------------------------------

%-------------------------------------
% These here are nice additional settings
% which may be changed if required
%-------------------------------------
\DefineBibliographyStrings{ngerman}{
  andothers = {et\addabbrvspace al\adddot} % ersetzt "u. a." durch "et al."
}
\renewcommand*{\nameyeardelim}{\addcomma\space %fügt Komma zwischen Autor und Jahr ein
}

% settings for references and bibliography already preloaded in preamble
%here: only individual settings
\ExecuteBibliographyOptions{
    sorting=nyt, %Sortierung Autor, Titel, Jahr
    bibwarn=true, %Probleme mit den Daten, die Backend betreffen anzeigen
    isbn=false, %keine isbn anzeigen
    url=true,
    maxnames=2,       % max. angezeigte Autoren
    minnames=1,       % min. angezeigte Autoren
    maxbibnames=99,   % im Literaturverzeichnis alle anzeigen
    uniquename=false, % keine künstlichen Namensabkürzungen
    giveninits=true   % Vornamen nur mit Initialen
}
\newcommand{\pc}{\parencite}
\newcommand{\tc}{\textcite}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
\usepackage{array}
\usepackage{ragged2e}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{pdflscape}
\usepackage{longtable}  
\usepackage{xltabular} 
\usepackage{csvsimple-l3}
\usepackage{makecell}
\usepackage{arydshln}
\usepackage{multirow}
\usepackage{graphicx}
\usepackage{pgfplotstable}
% Luft vor/nach longtable
\setlength{\LTpre}{0.75\baselineskip}   % Abstand vor der Tabelle (nur auf erster Seite)
\setlength{\LTpost}{0.5\baselineskip}   % Abstand nach der Tabelle (auf letzter Seite)
\setlength{\LTcapwidth}{\textwidth}     % Caption auf Textbreite
\newcolumntype{L}[1]{>{\RaggedRight\arraybackslash}m{#1}}
\newcolumntype{C}[1]{>{\Centering\arraybackslash}m{#1}}

% Header-Helfer (fett, links, Umbrüche erlaubt):
\newcommand{\LH}[1]{\makecell[l]{\bfseries #1}}
\newcommand{\rotH}[1]{\rotatebox[origin=c]{90}{\makecell[c]{#1}}}
\renewcommand{\arraystretch}{1.12}

\usepackage{caption}
\captionsetup{                  % globale Option für caption und subcaption
      font=normalsize,              % Schrift der Caption (Label+Text)
      format=hang,                  % Formatierung der Caption
      justification=RaggedRight,    % linksbündig bei mehreren caption-Zeilen
      singlelinecheck=true,         % true: einzelne Linie zentriert!
      labelfont=bf,                 % Schrift des Labels
      textfont=rm,                  % Schrift des Textes
      position=bottom               % Normale Caption unter dem Float
}
\usepackage{subcaption}
\usepackage{url}  
\usepackage{amsmath}
\numberwithin{equation}{chapter} 
\usepackage[colorlinks=true,linkcolor=blue, citecolor=magenta]{hyperref}
\usepackage{adjustbox}


\usepackage{rotating} % sidewaystable (dreht die Tabelle, Seite bleibt hochkant)
\usepackage{acronym} %define acronyms and list them in a glossary
\usepackage{siunitx}[=v2]   % Einheitenmakros, Nummernformatierung
\sisetup{locale=DE,binary-units = true} %Bit als Einheit
\usepackage{enumitem}

    
%-------------------------------------
% The main document starts here
%-------------------------------------
\begin{document}
\input{_basic_settings_dont_touch/frontmatter}
\input{your_content_folder/Abkuerzungsverzeichnis}
\input{your_content_folder/Symbolverzeichnis.tex}
\mainmatter
\input{your_content_folder/1_}
\input{your_content_folder/2_}
\input{your_content_folder/3_}
\input{your_content_folder/4_}
\input{your_content_folder/5_}
\input{your_content_folder/6_}
\input{your_content_folder/7_}
\printbibliography 


% do not change: sets specific page numbering for annex
\pretocmd{\chapter}{
    \clearpage
    \setcounter{page}{1}
}{}{}
\let\oldpagestyle=\thepage
\renewcommand*{\thepage}{\thechapter-\arabic{page}}
%----------------------------------------------------

% add annexures here
\appendix
\input{your_content_folder/App_A}
\input{your_content_folder/App_B}

%--------------------------------------
% you shouldn't make any changes from here
%--------------------------------------

\clearpage
\renewcommand*{\thepage}{\oldpagestyle}
\input{_basic_settings_dont_touch/eidesstatt}
\end{document}
